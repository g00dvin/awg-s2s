...
значения как значения по умолчанию; при наличии черновика он имеет приоритет.
Проверьте поля, нажимая Enter для сохранения значения либо вводя новое.
...
Мастер всегда добавляет туннельный `/32` другого сервера, нормализует сетевые
...
IP, допускаются с уведомлением, а не завершают скрипт. При `Table = off`
