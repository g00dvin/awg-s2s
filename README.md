...
- [Сохранение параметров и повторный запуск](#сохранение-параметров-и-повторный-запуск)
...
Мастера пока англоязычные. Начиная с `1.3.0`, настройки разбиты на короткие
секции, а итоговая конфигурация показана отдельными таблицами-карточками
серверов: длинные значения переносятся и не ломают общую таблицу.
Необязательные SSH-настройки ключа/jump host скрыты за отдельным вопросом.
В меню выбирайте номер. Для обычных вопросов yes/no используются `yes`/`y`
и `no`/`n`. Применение в финальном меню не выбрано по умолчанию.

Readline поддерживает редактирование строки и вставку CIDR-списков.
Цвет используется только в подходящем терминале; `NO_COLOR=1` отключает его.
В логах и при `TERM=dumb` оформление остаётся обычным текстом.

Начиная с `1.3.1`, controller передаёт скрипт через стандартный ввод SSH,
а не внутри аргумента команды. Это устраняет `Argument list too long` при
размещении большого скрипта. Временный файл доступен только SSH-пользователю,
затем root/sudo проверяет синтаксис и устанавливает системную копию с правами
`0700`. После установки или ошибки временная копия удаляется. Механизм
работает и для SSH-пользователя без прямого входа root.
...
6. Отдельный ключ/ProxyJump — только при выборе дополнительных SSH-настроек.
...
Начиная с версии `1.2.0`, **до выполнения команд на серверах** мастер собирает
также AllowedIPs каждой стороны и выбор firewall, а затем показывает таблицу:
сервер/способ подключения, интерфейс, туннельный IP, AllowedIPs, UDP-endpoint,
firewall и keepalive. SSH-ключ и jump host показываются отдельно, без секретов.
Применение начинается только после подтверждения этой таблицы. Подтверждённые
параметры сначала сохраняются в файл, поэтому ошибка установки не теряет план.
Финальное меню: `1 Save and apply`, `2 Edit settings`, `3 Exit without changes`.
При выборе редактирования команды на серверах не выполняются.

Перед удалённой или локальной операцией выводится метка назначения, например
`[listener: root@listener.example.org (SSH port 2222)]` или
`[connector: vpn-gateway (this machine)]`, а затем действие `install`, `up`
и т. п. Сам скрипт на сервере также сообщает фактическое имя машины.
Содержимое команд передачи секретов не печатается.
...
Controller по умолчанию сохраняет план в
`~/.config/awg-s2s/controller.plan`, локальный wizard — в
`~/.config/awg-s2s/wizard.plan` пользователя, запускающего мастер.
При запуске wizard через sudo это обычно домашний каталог root.
Если задан `XDG_CONFIG_HOME`, используется его подкаталог `awg-s2s`.

После каждого корректно введённого поля атомарно обновляется соседний файл `.draft`.
Так сохраняются даже частично введённые параметры при Ctrl+C, закрытии ввода
или ошибке. Черновик не означает согласие на выполнение команд.

После подтверждения таблицы атомарно записывается файл `.plan`, а черновик
удаляется. При следующем запуске мастер предлагает использовать сохранённые
значения; при наличии черновика он имеет приоритет. Меню предлагает:
`1 Review saved setup` — сразу перейти к обзору полного корректного плана;
`2 Edit saved values` — пройти поля со старыми значениями;
`3 Start fresh` — ввести всё заново. Неполные/некорректные сохранённые поля
мастер запрашивает отдельно. Применение всегда требует финального выбора.
При редактировании нажимайте Enter для сохранения значения либо вводите новое.
Чтобы очистить сохранённое необязательное SSH-поле, введите `-`.
После редактирования таблица снова требует подтверждения.

Файлы имеют права `0600`. Они содержат параметры подключения, имя пользователя,
пути к SSH-ключу/пакету и параметры туннеля, **но не пароли, приватные ключи,
header-protection key или PSK**. Формат данных не исполняется как shell-код.
Пути и адреса всё равно являются административной информацией: не публикуйте
планы без необходимости. Расширения `.plan` и `.plan.draft` добавлены в `.gitignore`.

Для нескольких независимых планов укажите отдельный файл:

```bash
AWG_PLAN_FILE="$HOME/.config/awg-s2s/office.plan" bash ./amnezia-site-to-site.sh controller
```

Отказ от подтверждения оставляет черновик, но не меняет последний подтверждённый
план и не выполняет настройки серверов. Отказ от загрузки сохранённых значений
позволяет начать ввод заново. Для сброса удалите нужный файл плана и его `.draft`;
установленная серверная конфигурация от этого не удаляется.

### AllowedIPs для каждой стороны

Вводите строку IPv4 CIDR через запятую или пробел, например
`172.16.0.0/12,10.0.0.0/8`. **На listener указываются адреса за connector;
на connector — адреса за listener.** Это настройки пира, а не список собственных
локальных сетей сервера.

| Сервер | Пример AllowedIPs | Значение |
| --- | --- | --- |
| Listener | `10.203.77.2/32,192.168.20.0/24` | Connector и сеть за ним |
| Connector | `10.203.77.1/32,172.16.0.0/16` | Listener и сеть за ним |

Мастер обеспечивает покрытие адреса пира в AllowedIPs: добавляет его `/32`,
только если адрес ещё не покрыт введённой сетью. При `/0` дополнительный `/32`
не нужен и не показывается. Мастер нормализует сетевые
адреса и убирает точные дубликаты. IPv6 пока не поддерживается.
...
настраиваете вручную, и сохраните независимый административный доступ.

При неправильном значении мастер показывает ошибку и повторяет **только это
поле**, сохраняя ранее введённые параметры. Это относится к пунктам меню,
SSH-полям, имени интерфейса, IP/порту, AllowedIPs, пути/формату пакета и
вводимому публичному ключу. После ошибки недопустимое значение не предлагается
как новое значение по умолчанию и не записывается в черновик.
Для завершения используйте Ctrl+C. Ошибки реальной установки, SSH-соединения
или повреждённого файла плана по-прежнему останавливают действие: скрипт не
скрывает системные ошибки и не пытается автоматически повторять изменения.

AllowedIPs выбирает пира для пакета, уже направленного в AWG-интерфейс, и
ограничивает допустимые исходные адреса пакетов от пира. При `Table = off`
системный маршрут этим параметром не создаётся. `/0` разрешает любые IPv4-адреса
в этих проверках, но не отменяет firewall. **Это не firewall-разрешение.** Добавление LAN-префикса не
включает forwarding, NAT, возвратные маршруты или доступ LAN к серверу:
их администратор настраивает отдельно. Автоматический firewall по-прежнему
разрешает локальные сервисы только от туннельного IP пира, не от всей его LAN.

Для автоматизации задайте значение отдельно на каждом сервере:

```bash
sudo env AWG_ALLOWED_IPS=10.203.77.2/32,192.168.20.0/24 bash ./amnezia-site-to-site.sh listener listener.example.org 51830 10.203.77.1 10.203.77.2
sudo env AWG_ALLOWED_IPS=10.203.77.1/32,172.16.0.0/16 bash ./amnezia-site-to-site.sh connector ./listener-bundle.txt
```

Пакет `AWG-SITE-V3` не содержит LAN-префиксов: каждая сторона выбирает их отдельно.
На повторном запуске несовпадающие AllowedIPs существующего профиля не
перезаписываются. Для изменения требуется согласованное ручное обслуживание
либо новый профиль; перезапуск мастера не является неявной миграцией.
...
ограничения внешних output/forward-цепочек. Локальный wizard также предлагает
выбор UFW/nftables/ручного режима до таблицы и применяет подтверждённый выбор.
Оба мастера отдельно просят подтвердить готовность host/provider firewall.
...
| `Table` | `off` | Системные маршруты туннеля автоматически не добавляются |
| `AllowedIPs` | IP другого конца с `/32` и выбранные сети | Разрешённые адреса пира и направление трафика |
...
`Table = on` — недопустимое значение awg-quick. Проект использует `Table = off`:
маршруты для AllowedIPs не создаются автоматически, в том числе для `/0`.
Если самостоятельно поставить `auto`, awg-quick начнёт устанавливать маршруты,
а `/0` может перенаправить интернет-трафик и изменить policy routing.
...
```bash
sudo ip route replace 10.203.77.2/32 dev awg-site
```
...
При `/0` не направляйте публичный endpoint обратно в тот же туннель: учитывайте
отдельный маршрут к endpoint и правила вашей policy routing-схемы.
...
| `allowed-ips` | Нормализованные AllowedIPs данного серверного профиля |
...
Начиная с версии скрипта `1.1.1`, уже существующее дерево headers используется
без повторного требования доступности пакета в APT. Если headers отсутствуют,
скрипт проверяет наличие кандидата пакета для запущенного ядра. При отсутствии
кандидата установка останавливается с инструкцией, не обновляя ядро автоматически.

Например, ошибка `Unable to locate package linux-headers-6.12.74+deb13+1-amd64`
означает, что APT не находит headers этого ядра в подключённых репозиториях.
Установка headers другой версии без загрузки соответствующего ядра не поможет.
Для стандартного Debian на amd64 в согласованное окно обслуживания:
...
Перед перезагрузкой проверьте наличие консоли провайдера и условия загрузки
сервера. Для нестандартного ядра сначала уточните его происхождение: может
потребоваться пакет headers из исходного доверенного репозитория. После
перезагрузки проверьте `uname -r` и повторите controller с теми же параметрами.
Уже выполненные действия на другом сервере сохраняются.
