...
После каждого введённого поля атомарно обновляется соседний файл `.draft`.
...
Вводите IPv4 CIDR через запятую. **На listener указываются адреса за connector;
...
адреса и убирает точные дубликаты. IPv6, маршрут `/0` и префиксы, содержащие
собственный туннельный IP, отклоняются. Проверяйте пересечения с другими
локальными маршрутами самостоятельно.
