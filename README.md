...
адреса и убирает точные дубликаты. IPv6 и маршрут `/0` отклоняются.
Начиная с версии `1.2.1`, широкие префиксы, содержащие собственный туннельный
IP, допускаются с предупреждением, а не завершают скрипт. Проверьте маршруты:
более специфичные локальные маршруты имеют приоритет, но прочий трафик к
широкой сети может уйти через пир. Сохраните независимый административный доступ.
...
AllowedIPs управляет маршрутами к пиру и допустимыми исходными адресами
пакетов от него. **Это не firewall-разрешение.** Добавление LAN-префикса не
...
всего интернета: маршруты ограничены `/32` пира и явно выбранными сетями.
При `Table = off` нужные маршруты требуется установить вручную.
